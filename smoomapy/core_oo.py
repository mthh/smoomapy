...
class SmoothStewart:
    def __init__(self, input_geojson_points, variable_name, span, beta,
                 typefct='exponential', resolution=None,
                 variable_name2=None, mask=None):
        self.gdf = GeoDataFrame.from_file(
            input_geojson_points).to_crs(crs="+proj=natearth")
        self.info = (
            'SmoothStewart - variable : {} ({} features)\n'
            'beta : {} - span : {} - function : {}'
            ).format(variable_name, len(self.gdf), beta, span, beta)

        if mask:
            self.mask = GeoDataFrame.from_file(
                mask).to_crs(crs="+proj=natearth") \
                    if mask != input_geojson_points else self.gdf

            if len(set(self.gdf.type
        ...
        self.compute_pot(variable_name, span, beta,
                         resolution=resolution, typefct=typefct)
    ...
    def render(self, nb_class=8, disc_func=None,
        ...
        if func_grid == "scipy":
            ...
            rbf = Rbf(self.x, self.y, self.pot, epsilon=2)
        ...
        if user_defined_breaks:
            ...
            if not disc_func or "prog_geom" in disc_func \
                ...
                if nb_class > 15:
                    ...
                    q = np.concatenate(
                        ([0], np.linspace(12.5, 98, nb_class-1), [100.0]))

                levels = np.percentile(
                    np.concatenate((pot[pot.nonzero()], np.array([0.0]))), q)
                ...
                levels = jenks_breaks(np.concatenate(
                    ([0], pot[pot.nonzero()])), nb_class)
            else:
                # Fallback on the method used by contourf function,
                # given a number of class :
                levels = nb_class

        collec_poly = contourf(
            self.xi, self.yi, self.zi,
            levels,
            vmax=abs(self.zi).max(), vmin=-abs(self.zi).max())
